...
Routes live in `app/api/**/route.ts`, validate input with `zod`, and return `NextResponse.json`. Wrap handlers with `apiHandler` (`lib/api-handler.ts`) for try/catch, or hand-roll the try/catch as the tasks route does. Heavy domain logic belongs in `lib/` (e.g. `lib/intake-pipeline.ts` is the single intake execution path; routes like `/api/intake`, `/api/intake/slack` are thin wrappers). Error responses carry a `code` (`unauthorized`/`forbidden`/`not_found`/`invalid_workspace`/`validation_error`) on the newer harness-facing routes.

### Harness onboarding (agents wiring into Cockpit)
External agents follow `~/workspaces/_shared/agent-protocols/cockpit-wiring.md`. Supporting endpoints: **`GET /api/whoami`** (verify bearer, echo harness/model/session identity + scopes, validate a `?workspace=` hint), **`GET /api/workspaces`** (canonical IDs from `lib/workspaces.ts` — the source of truth, since `tasks.workspaceId` is free text), and **`POST /api/tasks/[id]/events`** (append a structured progress/artifact/verification event without changing status). `POST /api/tasks` validates `workspaceId` against the workspaces table (422 `invalid_workspace`) and honours an `Idempotency-Key` header / `idempotencyKey` body field. Auth is still the single shared `CRON_SECRET` bearer (admin) — scoped per-harness tokens are the planned next step.
