...
import { Plus, X, TrendingUp, CheckCircle, Zap, DollarSign, Shield, Users, Calendar } from 'lucide-react'
...
type PeriodFilter = 'this-week' | 'this-month' | 'last-month' | 'all'

const REPORTING_PHASES = [
  { key: 'phase1', label: 'Phase 1: Week 1–2', desc: 'Daily reporting' },
  { key: 'phase2', label: 'Phase 2: Week 3–4', desc: 'Weekly reporting' },
  { key: 'phase3', label: 'Phase 3: Month End', desc: 'Full COPIL dashboard' },
]
...
function LogEntryDialog({ onClose, onSaved }: LogDialogProps) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (res.ok) {
        const row = await res.json() as MetricRow
        onSaved(row)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-[#1A1A1A] border border-[rgba(255,255,255,0.10)] rounded-[12px] overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className={labelCls}>Tasks Done</label>
              <input type="number" value={tasksCompleted} onChange={e => setTasksCompleted(e.target.value)}
                placeholder="0" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Tasks Total</label>
              <input type="number" value={tasksTotal} onChange={e => setTasksTotal(e.target.value)}
                placeholder="0" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Avg Duration (min)</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelCls}>Automation Rate (%)</label>
              <input type="number" min="0" max="100" value={automationRate} onChange={e => setAutomationRate(e.target.value)}
                placeholder="85" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>API Cost (USD)</label>
              <input type="number" step="0.01" value={apiCost} onChange={e => setApiCost(e.target.value)}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelCls}>Emails Sent</label>
              <input type="number" value={emailsSent} onChange={e => setEmailsSent(e.target.value)}
                placeholder="0" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Emails Received</label>
              <input type="number" value={emailsReceived} onChange={e => setEmailsReceived(e.target.value)}
                placeholder="0" className={inputCls} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className={labelCls}>Human Intervention (%)</label>
              <input type="number" min="0" max="100" value={humanIntervention} onChange={e => setHumanIntervention(e.target.value)}
                placeholder="15" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Client Satisfaction</label>
              <select value={clientSatisfaction} onChange={e => setClientSatisfaction(e.target.value)}
                className={inputCls + ' appearance-none'}>
            ...
            <div>
              <label className={labelCls}>Security Incidents</label>
              <input type="number" min="0" value={securityIncidents} onChange={e => setSecurityIncidents(e.target.value)}
                className={inputCls} />
            </div>
          </div>
          ...
          <div>
            ...
            <select value={reportingPhase} onChange={e => setReportingPhase(e.target.value as 'daily' | 'weekly' | 'copil')}
              className={inputCls + ' appearance-none'}>
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} placeholder="Add context, issues, highlights..."
...
// ── Main Client ───────────────────────────────────────────────────────────────
export function AIMetricsClient({ initialMetrics, recentTasks }: AIMetricsClientProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return metrics.filter(m => {
      ...
      if (periodFilter === 'this-week') return start >= startOfWeek
      if (periodFilter === 'this-month') return start >= startOfMonth
      if (periodFilter === 'last-month') return start >= startOfLastMonth && start <= endOfLastMonth
      return true
    })
  ...
  // ── Derived stats ────────────────────────────────────────────────────────
  const stats = useMemo(() => {
    const totalDone = filtered.reduce((s, m) => s + (m.tasksCompleted ?? 0), 0)
    const totalCost = filtered.reduce((s, m) => s + Number(m.apiCostUsd ?? 0), 0)
    const avgAutomation = filtered.length
      ? filtered.reduce((s, m) => s + Number(m.automationRate ?? 0), 0) / filtered.length
      : 0
    const totalSecurity = filtered.reduce((s, m) => s + (m.securityIncidents ?? 0), 0)
    const avgIntervention = filtered.length
      ? filtered.reduce((s, m) => s + Number(m.humanInterventionRate ?? 0), 0) / filtered.length
      : 0
    ...
    return {
      ...
      totalSecurity,
      avgIntervention,
      costPerTask: totalDone > 0 ? totalCost / totalDone : 0,
    }
  ...
  const chartData = useMemo(() => {
    return [...filtered]
      ...
      .map(m => ({
        date: m.periodStart.slice(5), // MM-DD
        completed: m.tasksCompleted ?? 0,
        automation: Number(m.automationRate ?? 0),
        cost: Number(m.apiCostUsd ?? 0),
      }))
  }, [filtered])
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[#6B7280] mt-0.5">Performance KPIs for business case reporting</p>
        </div>
        <button
          ...
          <button
            ...
            className={`px-3 py-1.5 text-xs rounded-[6px] border transition-colors ${
              periodFilter === opt.key
                ...
                : 'bg-transparent border-[rgba(255,255,255,0.06)] text-[#6B7280] hover:text-[#A0A0A0]'
            }`}
          >
            {opt.label}
          </button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
        <div className={statCard}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-[#6B7280] uppercase tracking-wide">Tasks Completed</span>
          </div>
          <p className="text-3xl font-bold text-[#F5F5F5]">{stats.totalDone}</p>
          <p className="text-xs text-[#4B5563] mt-1">{stats.dbTasksDone} in DB (all time)</p>
        </div>
        ...
        <div className={statCard}>
          ...
          <p className="text-3xl font-bold text-[#F5F5F5]">{stats.avgAutomation.toFixed(0)}<span className="text-lg text-[#6B7280]">%</span></p>
          <p className="text-xs text-[#4B5563] mt-1">avg across period</p>
        </div>
        ...
        <div className={statCard}>
          ...
          <p className="text-3xl font-bold text-[#F5F5F5]">${stats.totalCost.toFixed(2)}</p>
          <p className="text-xs text-[#4B5563] mt-1">${stats.costPerTask.toFixed(3)}/task</p>
        </div>
        ...
        <div className={statCard}>
          ...
          <p className="text-3xl font-bold text-[#F5F5F5]">{stats.avgIntervention.toFixed(0)}<span className="text-lg text-[#6B7280]">%</span></p>
          <p className="text-xs text-[#4B5563] mt-1">avg correction rate</p>
        </div>

        <div className={statCard}>
          <div className="flex items-center gap-2 mb-2">
            <Shield className="w-4 h-4 text-[#22C55E]" />
            <span className="text-xs text-[#6B7280] uppercase tracking-wide">Security Incidents</span>
          </div>
          <p className={`text-3xl font-bold ${stats.totalSecurity > 0 ? 'text-[#EF4444]' : 'text-[#22C55E]'}`}>{stats.totalSecurity}</p>
          <p className="text-xs text-[#4B5563] mt-1">target: 0</p>
        </div>

        <div className={statCard}>
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp className="w-4 h-4 text-[#A855F7]" />
            <span className="text-xs text-[#6B7280] uppercase tracking-wide">Entries Logged</span>
          </div>
          <p className="text-3xl font-bold text-[#F5F5F5]">{filtered.length}</p>
          <p className="text-xs text-[#4B5563] mt-1">{metrics.length} total all time</p>
        </div>
      </div>
      ...
      {chartData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="rounded-[8px] bg-[#1A1A1A] border border-[rgba(255,255,255,0.06)] p-4">
            <h3 className="text-sm font-semibold text-[#F5F5F5] mb-4">Tasks Completed</h3>
          ...
          {/* Automation rate line chart */}
          <div className="rounded-[8px] bg-[#1A1A1A] border border-[rgba(255,255,255,0.06)] p-4">
            <h3 className="text-sm font-semibold text-[#F5F5F5] mb-4">Automation Rate (%)</h3>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={chartData} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
                ...
                <Tooltip content={<CustomTooltip />} />
                <Line type="monotone" dataKey="automation" stroke="#22C55E" dot={false} strokeWidth={2} name="Automation %" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="rounded-[8px] bg-[#1A1A1A] border border-[rgba(255,255,255,0.06)] p-4 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {REPORTING_PHASES.map(phase => (
            <div key={phase.key} className="rounded-[6px] bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.06)] p-3">
      ...
      {metrics.length > 0 && (
        <div className="rounded-[8px] bg-[#1A1A1A] border border-[rgba(255,255,255,0.06)] overflow-hidden">
          <div className="px-4 py-3 border-b border-[rgba(255,255,255,0.06)]">
            <h3 className="text-sm font-semibold text-[#F5F5F5]">Logged Metrics Entries</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
      ...
      {showLog && (
        <LogEntryDialog onClose={() => setShowLog(false)} onSaved={row => setMetrics(prev => [row, ...prev])} />
      )}
    </div>
  )
}
