  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-[26px] font-medium text-[#E8DFCE]">AI Operations Metrics</h1>
          <p className="text-sm text-[#7A6F55] mt-0.5">Live from canonical operational log (last 90 days)</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
        <div className={statCard}>
          <div className="flex items-center gap-2 mb-2">
            <CheckCircle className="w-4 h-4 text-[#7D9B5E]" />
            <span className="text-xs text-[#7A6F55] uppercase tracking-wide">Agent Events</span>
          </div>
          <p className="text-3xl font-bold text-[#E8DFCE]">{liveStats.totalEvents}</p>
          <p className="text-xs text-[#5C5340] mt-1">{dbTasksDone} tasks done in DB</p>
        </div>

        <div className={statCard}>
          <div className="flex items-center gap-2 mb-2">
            <Zap className="w-4 h-4 text-[#5F7A72]" />
            <span className="text-xs text-[#7A6F55] uppercase tracking-wide">Automation Rate</span>
          </div>
          <p className="text-3xl font-bold text-[#E8DFCE]">{liveStats.automationRate.toFixed(0)}<span className="text-lg text-[#7A6F55]">%</span></p>
          <p className="text-xs text-[#5C5340] mt-1">no human intervention</p>
        </div>

        <div className={statCard}>
          <div className="flex items-center gap-2 mb-2">
            <DollarSign className="w-4 h-4 text-[#C99A1F]" />
            <span className="text-xs text-[#7A6F55] uppercase tracking-wide">API Cost</span>
          </div>
          <p className="text-3xl font-bold text-[#E8DFCE]">${liveStats.totalCost.toFixed(2)}</p>
          <p className="text-xs text-[#5C5340] mt-1">${liveStats.costPerEvent.toFixed(3)}/event</p>
        </div>

        <div className={statCard}>
          <div className="flex items-center gap-2 mb-2">
            <Users className="w-4 h-4 text-[#C96F2E]" />
            <span className="text-xs text-[#7A6F55] uppercase tracking-wide">Human Intervention</span>
          </div>
          <p className="text-3xl font-bold text-[#E8DFCE]">{liveStats.interventionRate.toFixed(0)}<span className="text-lg text-[#7A6F55]">%</span></p>
          <p className="text-xs text-[#5C5340] mt-1">{liveStats.interventionCount} events required review</p>
        </div>

        <div className={statCard}>
          <div className="flex items-center gap-2 mb-2">
            <Bot className="w-4 h-4 text-[#9B6B4F]" />
            <span className="text-xs text-[#7A6F55] uppercase tracking-wide">Active Agents</span>
          </div>
          <p className="text-3xl font-bold text-[#E8DFCE]">{agentBreakdown.length}</p>
          <p className="text-xs text-[#5C5340] mt-1">{liveStats.totalTokens.toLocaleString()} tokens used</p>
        </div>

        <div className={statCard}>
          <div className="flex items-center gap-2 mb-2">
            <TrendingUp className="w-4 h-4 text-[#4A8578]" />
            <span className="text-xs text-[#7A6F55] uppercase tracking-wide">Event Families</span>
          </div>
          <p className="text-3xl font-bold text-[#E8DFCE]">{familyBreakdown.length}</p>
          <p className="text-xs text-[#5C5340] mt-1">distinct operational domains</p>
        </div>
      </div>
          ...
          <div className="rounded-none bg-[#281E16] border border-[rgba(167,155,120,0.13)] p-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={dailySeries} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(167,155,120,0.09)" />
                <XAxis dataKey="date" tick={{ fill: '#7A6F55', fontSize: 11 }} />
                <YAxis tick={{ fill: '#7A6F55', fontSize: 11 }} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="count" fill="#5F7A72" radius={[3, 3, 0, 0]} name="Events" />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="rounded-none bg-[#281E16] border border-[rgba(167,155,120,0.13)] p-4">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={dailySeries} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
                ...
                <YAxis tick={{ fill: '#7A6F55', fontSize: 11 }} />
                <Tooltip content={<CustomTooltip />} />
                <Line type="monotone" dataKey="cost" stroke="#C99A1F" dot={false} strokeWidth={2} name="Cost ($)" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="rounded-none bg-[#281E16] border border-[rgba(167,155,120,0.13)] p-4 mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {agentBreakdown.map(a => {
              ...
              return (
                <div key={a.agentId} className="rounded-none bg-[rgba(167,155,120,0.07)] border border-[rgba(167,155,120,0.13)] p-3">
                  <div className="flex items-center gap-2 mb-2">
                    {def && <span className="text-sm">{def.emoji}</span>}
                    <span className="text-sm font-medium" style={{ color: def?.color ?? '#A79B78' }}>
                      {def?.name ?? a.agentId}
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <p className="text-xs text-[#5C5340]">Events</p>
                      <p className="text-sm font-mono text-[#E8DFCE]">{a.count}</p>
                    </div>
                    <div>
                      <p className="text-xs text-[#5C5340]">Cost</p>
                      <p className="text-sm font-mono text-[#E8DFCE]">${a.cost.toFixed(2)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-[#5C5340]">Auto %</p>
                      <p className="text-sm font-mono text-[#E8DFCE]">{autoRate}%</p>
                    </div>
                  </div>
                </div>
              )
              ...
              {familyBreakdown.map(f => {
                ...
                return (
                  <div key={f.family} className="flex items-center gap-3">
                    <span className="text-xs text-[#A79B78] w-24 shrink-0 capitalize">{f.family}</span>
                    <div className="flex-1 h-2 rounded-full bg-[rgba(167,155,120,0.13)] overflow-hidden">
                      <div className="h-full rounded-full bg-[#5F7A72]" style={{ width: `${(f.count / max) * 100}%` }} />
                    </div>
                    <span className="text-xs font-mono text-[#7A6F55] w-6 text-right">{f.count}</span>
                  </div>
                )
              })}
            </div>
              ...
              {entityBreakdown.map(e => {
                ...
                return (
                  <div key={e.entity} className="flex items-center gap-3">
                    <span className="text-xs text-[#A79B78] w-24 shrink-0">{ENTITY_LABELS[e.entity] ?? e.entity}</span>
                    <div className="flex-1 h-2 rounded-full bg-[rgba(167,155,120,0.13)] overflow-hidden">
                      <div className="h-full rounded-full bg-[#C99A1F]" style={{ width: `${(e.count / max) * 100}%` }} />
                    </div>
                    <span className="text-xs font-mono text-[#7A6F55] w-6 text-right">{e.count}</span>
                  </div>
                )
              })}
            </div>
      ...
      <div className="rounded-none bg-[#281E16] border border-[rgba(167,155,120,0.13)] p-4 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={phase.label} className="rounded-none bg-[rgba(167,155,120,0.07)] border border-[rgba(167,155,120,0.13)] p-3">
              <p className="text-xs font-medium text-[#E8DFCE] mb-0.5">{phase.label}</p>
              <p className="text-xs text-[#7A6F55]">{phase.desc}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-none bg-[#281E16] border border-[rgba(167,155,120,0.13)] overflow-hidden mb-8">
        ...
        <div className="overflow-x-auto">
            ...
            <thead>
              <tr className="border-b border-[rgba(167,155,120,0.13)]">
                {['Task', 'Status', 'Priority', 'Assignee', 'Tags', 'Created'].map(h => (
                  <th key={h} className="px-4 py-2 text-left text-xs font-medium text-[#7A6F55] uppercase tracking-wide">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {recentTasks.slice(0, 30).map(t => (
                <tr key={t.id} className="border-b border-[rgba(167,155,120,0.09)] last:border-0 hover:bg-[rgba(167,155,120,0.04)]">
                  <td className="px-4 py-2.5 text-sm text-[#E8DFCE] max-w-[200px] truncate">{t.title}</td>
                  <td className="px-4 py-2.5">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      ['Done', 'Completed'].includes(t.status) ? 'bg-[rgba(125,155,94,0.12)] text-[#7D9B5E]' :
                      t.status === 'In Progress' ? 'bg-[rgba(95,122,114,0.12)] text-[#5F7A72]' :
                      'bg-[rgba(167,155,120,0.13)] text-[#7A6F55]'
                    }`}>{t.status}</span>
                  </td>
                  <td className="px-4 py-2.5 text-xs text-[#7A6F55]">{t.priority ?? '—'}</td>
                  <td className="px-4 py-2.5 text-xs text-[#7A6F55]">{t.assignee ?? '—'}</td>
                  <td className="px-4 py-2.5 text-xs text-[#7A6F55]">{t.tags?.join(', ') ?? '—'}</td>
                  <td className="px-4 py-2.5 text-xs text-[#5C5340]">
          ...
          {recentTasks.length === 0 && (
            <p className="text-center text-sm text-[#5C5340] py-8">No tasks in the last 90 days</p>
          )}
        </div>
      </div>
              ...
              <thead>
                <tr className="border-b border-[rgba(167,155,120,0.13)]">
                  {['Period', 'Type', 'Done/Total', 'Automation', 'API Cost', 'Intervention', 'Security', 'Phase'].map(h => (
                    <th key={h} className="px-4 py-2 text-left text-xs font-medium text-[#7A6F55] uppercase tracking-wide whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {entries.map(m => (
                  <tr key={m.id} className="border-b border-[rgba(167,155,120,0.09)] last:border-0 hover:bg-[rgba(167,155,120,0.04)]">
                    <td className="px-4 py-2.5 text-xs text-[#E8DFCE] whitespace-nowrap">{m.periodStart} → {m.periodEnd}</td>
                    <td className="px-4 py-2.5 text-xs text-[#7A6F55] capitalize">{m.period}</td>
                    <td className="px-4 py-2.5 text-xs text-[#E8DFCE]">{m.tasksCompleted ?? '—'} / {m.tasksTotal ?? '—'}</td>
                    <td className="px-4 py-2.5 text-xs text-[#E8DFCE]">{m.automationRate ? `${m.automationRate}%` : '—'}</td>
                    <td className="px-4 py-2.5 text-xs text-[#E8DFCE]">{m.apiCostUsd ? `$${Number(m.apiCostUsd).toFixed(2)}` : '—'}</td>
                    <td className="px-4 py-2.5 text-xs text-[#E8DFCE]">{m.humanInterventionRate ? `${m.humanInterventionRate}%` : '—'}</td>
                    <td className={`px-4 py-2.5 text-xs font-medium ${(m.securityIncidents ?? 0) > 0 ? 'text-[#C0452E]' : 'text-[#7D9B5E]'}`}>
                      {m.securityIncidents ?? 0}
                    </td>
                    <td className="px-4 py-2.5 text-xs text-[#7A6F55] capitalize">{m.reportingPhase ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
