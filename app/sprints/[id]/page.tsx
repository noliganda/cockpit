...
export default async function SprintDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Group tasks by status for a mini board view
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex gap-4 overflow-x-auto pb-2">
          ...
          return (
            <div key={status} className="flex-shrink-0 w-56">
              <div className="flex items-center gap-2 mb-3">
                <h2 className="text-xs font-semibold text-[#A0A0A0] uppercase tracking-wide">{status}</h2>
                <span className="text-xs text-[#6B7280] font-mono">{statusTasks.length}</span>
              </div>
              <div className="space-y-2">
                {statusTasks.map(task => (
                  <div key={task.id} className="p-3 rounded-[8px] bg-[#141414] border border-[rgba(255,255,255,0.06)]">
        ...
        {sprintTasks.length === 0 && (
          <p className="text-sm text-[#4B5563]">No tasks in this sprint.</p>
        )}
      </div>
    </div>
  )
